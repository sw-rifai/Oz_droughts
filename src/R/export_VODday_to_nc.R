...
out_file_name <- "LPDRv2_VODday_monmean_2002_2019.nc"
out_dir <- "../data_general/Oz_misc_data/"
short_name <- "vod_day"
long_name <- "VOD daytime monthly mean"
units <- ""
title <- "LPDRv2 Vegetation Optical Depth Monthly Mean"
compression_level <- 6
...
pre_tc <- pre_tc %>% select(lon,lat,date,vod_day)
...
ncvar_put(ncout, def_var, tc$mets$vod_day)
