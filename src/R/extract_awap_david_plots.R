...
nvis <- read_csv("../Bauman_Aus/data/Australia_coordinates.csv")
nvis <- nvis %>% rename(x=lon,y=lat)
#*******************************************************************************
#* END SECTION
#*******************************************************************************
...
plot(nvis$x~coords_keep_awap$x)
plot(nvis$y~coords_keep_awap$y)
coords_keep_awap
...
out <- tmp_clim %>% select(-x.x,-y.x,-x.y,-y.y) %>% 
  rename(x=x_vi, y=y_vi) %>% 
  left_join(., nvis, by=c('x','y')) 

out %>% 
  ggplot(data=.,aes(date, t39,color=plot))+
  geom_line()+
  scale_color_viridis_d()+
  facet_wrap(~plot)

write_csv(out, "../Bauman_Aus/data/AWAP_David_Oz_plots.csv")
...
tmp_clim %>% filter(date==ymd("2000-01-01"))

inner_join(coords_dict, nvis)

coords_keep_awap$plot <- nvis$plot

  
unique(tmp_clim$x.y) %in% coords_keep_awap$x
...
out <- tmp_clim %>% 
  ...
  # select(-x_vi,-y_vi) %>% 
  # select(-y.x,-x.y) %>%
  left_join(coords_keep_awap,., by=c("x","y")) %>% 
  select(-ndvi_mcd,-vc)
out$plot %>% unique

out %>% 
  ggplot(data=.,aes(date, tmax,group=plot,color=plot))+
  geom_line()+
  facet_wrap(~plot)
  
apet %>% filter(time==min(time)) %>% ggplot(data=.,aes(x,y,color=pet))+geom_point()+
  geom_label(aes(label=plot))

out %>% filter(date==max(date,na.rm=TRUE)) %>% ggplot(data=.,aes(x,y,color=pet))+geom_point()+
  geom_label(aes(label=plot))

nvis %>% rename(x_vi=x,y_vi=y)
atmax %>% 
  left_join(., nvis %>% rename(x_vi=x,y_vi=y), by=c("x_vi","y_vi")) %>% 
  filter(time==max(time)) %>% 
  ggplot(data=.,aes(x_vi,y_vi,color=tmax))+
  geom_label(aes(label=plot))+
  coord_equal()
