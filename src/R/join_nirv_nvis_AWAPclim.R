...
# Anna's Priestly-Taylor calculation
########################################################################
coords <- dat %>% select(x,y) %>% 
  distinct() %>% 
  st_as_sf(coords = c("x","y"), crs=4326)
fp <- "../data_general/clim_grid/awap/AWAP/Monthly_data_1911_2019/AWAP_monthly_PriestleyTaylor_PET_1990_2019.nc"
vec_dates <- stars::read_ncdf(fp, var="time")
vec_dates <- tibble(time=st_get_dimension_values(vec_dates, 'time'))
...
########################################################################
# extract precip -------------------------------------------------------
# ALREADY DONE BUT LEAVING HERE JUST IN CASE
########################################################################
dprecip <- arrow::read_parquet("../data_general/clim_grid/awap/parquet/awap_precip_monthly_1900_2019_proc2020-04-12.parquet")
