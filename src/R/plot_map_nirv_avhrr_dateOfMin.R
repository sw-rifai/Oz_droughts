library(viridisLite)
library(raster); library(rasterVis);     #  LOAD THESE FIRST
library(sf); library(stars)
library(tidyverse); library(lubridate)   #  THEN LOAD TIDYWHATEVER STUFF
library(data.table)
library(dtplyr)
library(dplyr, warn.conflicts = FALSE);

o <- stars::read_stars("../data_general/AVHRR_NDVI_CDR_V5/AVHRR_CDRv5_NIRV_seasonalMedian_Australia_1982_2019.tif")
o

# Set the time dimension
vec_dates <- seq(ymd("1982-01-01"), ymd("2019-11-01"), by="3 months")
# raster::setZ(o, vec_dates, name='time')
stars::st_get_dimension_values(o, 'band')
o <- st_set_dimensions(o, 3, values = vec_dates, names = "date")


o <- o %>% 
  as.data.table() %>% 
  set_names(c("lon","lat","date","nirv")) %>% 
  lazy_dt(.) %>% 
  filter(is.na(nirv)==F)

o_norms <- o %>% 
  filter(date >= ymd("1982-01-1") & 
           date <= ymd("2011-12-31")) %>% 
  mutate(month=month(date)) %>% 
  group_by(lon,lat,month) %>% 
  summarize(nirv_u = mean(nirv, na.rm=TRUE), 
            nirv_sd = sd(nirv, na.rm=TRUE)) %>% 
  ungroup() %>% 
  as_tibble()

o <- inner_join(o %>% 
                  mutate(month=month(date)), 
                o_norms, by=c("lon","lat","month"))
o <- o %>% 
  mutate(nirv_anom = nirv - nirv_u)
o <- o %>% mutate(nirv_anom_sd = nirv_anom/nirv_sd)

ot <- o %>% as_tibble()

tmp <- ot %>% 
  group_by(lon, lat) %>% 
  mutate(nirv_pr = 100*rank(nirv_anom)/length(nirv_anom)) %>% 
  ungroup()


# Plot date of max & min nirv anom ----------------------------------------------
d_col_pal <- function(y){
  vec_cols <- viridisLite::inferno(5)
  case_when(y <1990 ~ vec_cols[1], 
            y >= 1990 & y<2000 ~ vec_cols[2],
            y >= 2000 & y < 2010 ~ vec_cols[3], 
            y >= 2010 & y <= 2013 ~ vec_cols[4],
            y >= 2017 ~ vec_cols[5], 
            TRUE ~ 'grey')}

# scales::muted(d_col_pal(1982:2020))
drought_epoch_theme <- rasterTheme(region=
                         (d_col_pal(1982:2020)),
                         fill='grey')
map_theme$panel.background$col = 'gray' 


d_dmin <- tmp %>% 
  mutate(q = case_when(month==1~"DJF",
                       month==4~"MAM",
                       month==7~"JJA",
                       month==10~"SON")) %>% 
  mutate(season = factor(q,levels=c("DJF","MAM","JJA","SON"), 
                         ordered=T)) %>% 
  group_by(lon,lat,season) %>% 
  filter(nirv_pr == min(nirv_pr)) %>%
  mutate(ddate = decimal_date(date))

p1 <- d_dmin %>% 
  filter(lat >= -44) %>% 
  filter(lon <= 154) %>% 
  ggplot(data=.,aes(lon,lat,fill=(ddate)))+
  geom_tile()+
  coord_equal()+
  labs(x=NULL, y=NULL, title='Year of Min. NIR-V Anomaly (1982-2019)', subtitle = 'AVHRR-CDRv5')+
  scale_fill_viridis_c("year",
                       option='B', direction = 1)+
  facet_wrap(~season)+
  theme_linedraw()
ggsave(p1, filename="figures/map_avhrr_nirv_season_date_min_anom.png",
       type='cairo',
       height=16, width=18, units='cm', dpi='retina')


r_dmax <- tmp %>% 
  group_by(lon,lat) %>% 
  filter(nirv_pr == 100) %>%
  mutate(ddate = decimal_date(date)) %>% 
  select(lon,lat,ddate) %>% 
  rasterFromXYZ(.) 

r_dmin <- tmp %>% 
  group_by(lon,lat) %>% 
  filter(nirv_pr == min(nirv_pr)) %>%
  mutate(ddate = decimal_date(date)) %>% 
  select(lon,lat,ddate) %>% 
  rasterFromXYZ(.) 

levelplot(r_dmax,
            # as.table=T,
            margin=F, 
            ylim=c(-44,-10), 
            xlim=c(112.5,154.5),
            # main='NIR-V Anomalies (AVHRR CDR v5)',
          par.settings=drought_epoch_theme,
          at=seq(1982,2020, length.out = 39),
          # layout = c(10, 4) # columns, rows
  )


p_dmin <- levelplot(r_dmin,
          # as.table=T,
          margin=F, 
          ylim=c(-44,-10), 
          xlim=c(112.5,154.5),
          main='Date of Min. NIR-V Anomaly (AVHRR CDR v5)',
          # par.settings=drought_epoch_theme,
          at=seq(1982,2020, length.out = 39),
          # layout = c(10, 4) # columns, rows
)
png(filename = 'figures/map_avhrr_nirv_date_min_anom.png', 
    width = 16, height = 12, units='cm',res=350,type='cairo'); p_dmin; dev.off()


# Plot seasonal anomalies rank percentile -------------------------------------------------
# convert to raster stack
r <- tmp %>% 
  mutate(year=year(date)) %>% 
  mutate(q = case_when(month==1~"DJF",
                       month==4~"MAM",
                       month==7~"JJA",
                       month==10~"SON")) %>% 
  mutate(season = factor(q,levels=c("DJF","MAM","JJA","SON"), 
                         ordered=T)) %>% 
  select(lon,lat,year,season,nirv_pr) %>% 
  na.omit() %>% 
  arrange(year, season) %>% 
  pivot_wider(#id_cols = c("lon","lat"), 
    names_from=c(year,season),
    values_from = nirv_pr) %>% 
  rasterFromXYZ(.)

# Divide into four time epochs
names(r)
vec_rn <- paste(rep(c("DJF","MAM", "JJA","SON"), 2019-1982+1),
                sort(rep(1982:2019,4))
)
vec_rn <- vec_rn[-length(vec_rn)]
names(r) <- vec_rn

map_theme <- rasterTheme(region=
  c("red",viridisLite::inferno(98,direction = -1),"blue"), 
                         fill='grey')
map_theme$panel.background$col = 'gray' 

# 1980s ------------------------------------------------------
p_80s <- levelplot(r,
                   layers=c(which(str_detect(string=vec_rn, pattern=c("(?=.*DJF*)"))&
                                    str_detect(string=vec_rn, pattern=c("(?=.198)"))), 
                            which(str_detect(string=vec_rn, pattern=c("(?=.*MAM*)"))&
                                    str_detect(string=vec_rn, pattern=c("(?=.198)"))),
                            which(str_detect(string=vec_rn, pattern=c("(?=.*JJA*)"))&
                                    str_detect(string=vec_rn, pattern=c("(?=.198)"))), 
                            which(str_detect(string=vec_rn, pattern=c("(?=.*SON*)"))&
                                    str_detect(string=vec_rn, pattern=c("(?=.198)")))),
                   as.table=T,
                   margin=F, 
                   ylim=c(-44,-10), 
                   xlim=c(112.5,154.5), 
                   # main='NIR-V Anomalies (AVHRR CDR v5)',
                   par.settings=map_theme,
                   at=seq(0,100,length.out = 100), 
                   layout = c(10, 4) # columns, rows
)
png(filename = 'figures/map_avhrr_nirv_seasonal_anoms_percentRank_1980s.png', 
    width = 40, height = 15, units='cm',res=350); p_80s; dev.off()

# 1990s ------------------------------------------------------
p_90s <- levelplot(r,
                   layers=c(which(str_detect(string=vec_rn, pattern=c("(?=.*DJF*)"))&
                                    str_detect(string=vec_rn, pattern=c("(?=.199)"))), 
                            which(str_detect(string=vec_rn, pattern=c("(?=.*MAM*)"))&
                                    str_detect(string=vec_rn, pattern=c("(?=.199)"))),
                            which(str_detect(string=vec_rn, pattern=c("(?=.*JJA*)"))&
                                    str_detect(string=vec_rn, pattern=c("(?=.199)"))), 
                            which(str_detect(string=vec_rn, pattern=c("(?=.*SON*)"))&
                                    str_detect(string=vec_rn, pattern=c("(?=.199)")))),
                   as.table=T,
                   margin=F, 
                   ylim=c(-44,-10), 
                   xlim=c(112.5,154.5), 
                   # main='NIR-V Anomalies (AVHRR CDR v5)',
                   par.settings=map_theme,
                   at=seq(0,100,length.out = 100), 
                   layout = c(10, 4) # columns, rows
)
png(filename = 'figures/map_avhrr_nirv_seasonal_anoms_percentRank_1990s.png', 
    width = 40, height = 15, units='cm',res=350); p_90s; dev.off()

# 2000s ------------------------------------------------------
p_00s <- levelplot(r,
                   layers=c(which(str_detect(string=vec_rn, pattern=c("(?=.*DJF*)"))&
                                    str_detect(string=vec_rn, pattern=c("(?=.200)"))), 
                            which(str_detect(string=vec_rn, pattern=c("(?=.*MAM*)"))&
                                    str_detect(string=vec_rn, pattern=c("(?=.200)"))),
                            which(str_detect(string=vec_rn, pattern=c("(?=.*JJA*)"))&
                                    str_detect(string=vec_rn, pattern=c("(?=.200)"))), 
                            which(str_detect(string=vec_rn, pattern=c("(?=.*SON*)"))&
                                    str_detect(string=vec_rn, pattern=c("(?=.200)")))),
                   as.table=T,
                   margin=F, 
                   ylim=c(-44,-10), 
                   xlim=c(112.5,154.5), 
                   # main='NIR-V Anomalies (AVHRR CDR v5)',
                   par.settings=map_theme,
                   at=seq(0,100,length.out = 100), 
                   layout = c(10, 4) # columns, rows
)
png(filename = 'figures/map_avhrr_nirv_seasonal_anoms_percentRank_2000s.png', 
    width = 40, height = 15, units='cm',res=350); p_00s; dev.off()

# 2010s ------------------------------------------------------
p_10s <- levelplot(r,
                   layers=c(which(str_detect(string=vec_rn, pattern=c("(?=.*DJF*)"))&
                                    str_detect(string=vec_rn, pattern=c("(?=.201)"))), 
                            which(str_detect(string=vec_rn, pattern=c("(?=.*MAM*)"))&
                                    str_detect(string=vec_rn, pattern=c("(?=.201)"))),
                            which(str_detect(string=vec_rn, pattern=c("(?=.*JJA*)"))&
                                    str_detect(string=vec_rn, pattern=c("(?=.201)"))), 
                            which(str_detect(string=vec_rn, pattern=c("(?=.*SON*)"))&
                                    str_detect(string=vec_rn, pattern=c("(?=.201)")))),
                   as.table=T,
                   margin=F, 
                   ylim=c(-44,-10), 
                   xlim=c(112.5,154.5), 
                   # main='NIR-V Anomalies (AVHRR CDR v5)',
                   par.settings=map_theme,
                   at=seq(0,100,length.out = 100), 
                   layout = c(10, 4) # columns, rows
)
png(filename = 'figures/map_avhrr_nirv_seasonal_anoms_percentRank_2010s.png', 
    width = 40, height = 15, units='cm',res=350); p_10s; dev.off()




















# SCRATCH -----------------------------------------------------------------


mtcars %>% 
  group_by(cyl) %>% 
  mutate(val = 100*rank(mpg)/length(mpg)) %>% 
  ungroup()

x <- rnorm(10)
100*rank(x)/length(x)
